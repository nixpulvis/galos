...
//! one list of readings — each entry and whoever said it, a commander's
//! journal and a published dump both — to each, and compares what they
//! published.
//!
...
//!   publishing it through `galos_index::store::names::Writer` — so the order *is*
...
//!   `galos_index::records::derive::lit` over exactly the stars compared here, and
...
/// `galos_index::accumulate::galaxy::UNKNOWN` would publish a different `updated_by`
