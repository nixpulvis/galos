...
/// them, and a star with no color index and no spectral type still has a
...
pub struct Star {
    ...
    /// `B-V` color index, where measured.
...
impl Star {
    ...
    /// giant is read as a dwarf of its letter — which is why a color index,
