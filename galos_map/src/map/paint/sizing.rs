...
//! [`crate::map::paint::field`], which paints it.

use crate::map::camera::OrbitCamera;
use crate::map::schedule::MapSet;

use crate::map::bodies::spawn::{Body, Unscanned, WORTH_KEEPING, WORTH_SIZING};
use crate::map::galaxy::System;
use crate::map::galaxy::spawn::{Shell, StarExposure};
use crate::map::paint::sphere::Roundness;
use crate::map::screen::{depth, depth_of, world_per_pixel};
use bevy::math::DVec3;
use bevy::prelude::*;
...
pub enum View {
    ...
    // the Milky Way — is still not drawn: `crate::map::paint::glow` lays the political
    ...
    // unwritten. `crate::map::galaxy::plan::Planned` already carries the splats.
    Realistic,
}
...
/// ([`crate::map::galaxy::visibility`]), what the pointer prefers between two marks
/// ([`crate::map::pointing`]), and whether a mark may be painted under a pixel
/// ([`crate::map::paint::field::floor`]). The realistic sky sizes stars by what they put
...
/// than how large each is: [`crate::map::galaxy::walk`] spends a cell's budget on the
...
/// [`crate::map::paint::field::floor`], which the map does without while it is scaling
...
/// scale: nobody living there is not a size, and [`crate::map::galaxy::visibility`] is what
...
///
/// Half a pixel is under [`crate::map::paint::field`]'s own floor, and that floor is the
...
/// the drawn view belongs to, and read by [`crate::map::paint::field`], which divides it
/// back out to pixels and paints there, and by
/// [`crate::map::pointing::size_indicators`], which rings it and catches the
/// pointer over it.
///
...
///
/// Held under the inverse of [`crate::map::bodies::spawn::WORTH_HIDING`], which is
...
///
/// The surface of the disc [`crate::map::paint::field`] paints once the system is near
...
/// shell stands for. [`crate::map::grid`] hands the ruled plane over across it, and
...
/// [`crate::map::bodies::spawn::WORTH_DRAWING`], which is where the contents of a
...
/// stands for. Nothing is drawn where it is: [`crate::map::paint::field`] reads the size
...
pub(crate) fn size_by_distance(
    ...
    mut shells: Query<
        (&mut Drawn, &System, &Visibility, Has<crate::map::route::Hop>),
        With<Shell>,
    >,
) {
    if !shells.is_empty() {
        ...
        for (mut drawn, system, visible, hop) in shells.iter_mut() {
            ...
            let away =
                crate::map::space::metres(eye - DVec3::from(system.position))
                    .length() as f32;
            ...
            let ordinary = mark_at(away)
                .max(crate::map::paint::field::SMALLEST * per_pixel);
...
///
/// A radius, so this is two pixels across. Below [`crate::map::pointing`]'s floor
...
///
/// Which is the other way round from [`crate::map::pointing::size_bodies`], and
...
/// [`crate::map::pointing`]'s floor for a body's mark, so a mark standing in for
...
/// for the reason [`crate::map::pointing::size_bodies`] is: `big_space` writes
...
pub fn size_marks(
    ...
    for (mark, of, cell, mut drawn, mut shown) in &mut marks {
        ...
        let place = system.position() + crate::map::space::light_years(metres);
        // A metre, which is as near as the camera may be pulled to anything.
...
/// painted flat in screen space now (see [`crate::map::labels::draw_names`]) and
/// inherits nothing from the star, so a star of no size would still be named
/// and still be aimed at — [`crate::map::pointing`] floors a system's mark at a
/// size for the hand whatever the field draws.
///
/// What reads it is the field. [`crate::map::paint::field::drawn_radius`] takes the size
...
///
/// [`crate::map::galaxy::spawn::star_psf`] cuts the Moffat with its core at an eighth of
...
/// [`crate::map::galaxy::spawn::star_psf`] actually cut. Past that the quad has no texture
...
///
/// A star is sized to the radius its point spread (`crate::map::galaxy::spawn::star_psf`)
...
/// size, which [`crate::map::paint::field`] takes back to pixels and paints the glint at.
///
...
pub(crate) fn size_photometrically(
    ...
    profile: Res<crate::map::galaxy::spawn::StarProfile>,
    ...
    for (mut drawn, system, visible) in shells.iter_mut() {
        ...
        let away = crate::map::space::metres(orbit.eye_from(system.position()))
            .length() as f32;
...
mod tests {
    use super::*;
    use crate::map::bodies::STAND_IN;
    use crate::map::galaxy::tests::{at, reaching};
    ...
    fn a_wide_system_is_a_mark_from_far_off() {
        let away = 100. * crate::map::space::LIGHT_YEAR as f32;
    ...
    fn a_system_is_sized_before_its_insides_are_drawn() {
        use crate::map::bodies::spawn::WORTH_DRAWING;
    ...
    fn a_shell_only_grows_on_screen_as_the_camera_comes_in() {
        for extent in [STAND_IN, 1e13, 1e14, 5e14, 1e15, 2.1e15] {
            let mut away = 2000. * crate::map::space::LIGHT_YEAR as f32;
    ...
    fn a_mark_is_gone_before_the_camera_reaches_the_shell() {
        let gone_at = 1. / crate::map::bodies::spawn::WORTH_HIDING;
    ...
    fn learning_how_far_a_system_reaches_leaves_its_mark_alone() {
        let away = 5. * crate::map::space::LIGHT_YEAR as f32;
    ...
    fn seen(ly: f32) -> f32 {
        let away = ly * crate::map::space::LIGHT_YEAR as f32;
    ...
    fn a_neighbour_is_a_mark_rather_than_a_sky() {
        ...
        let away = 0.1 * crate::map::space::LIGHT_YEAR as f32;
    ...
    fn pixels(app: &mut App, address: i64, by_population: bool) -> f32 {
        ...
        let away =
            crate::map::space::metres(eye - system.position()).length() as f32;
        let per_pixel = world_per_pixel(cot_half_fov, height, away.max(1.));

        crate::map::paint::field::drawn_radius(
            &View::Map,
            drawn.0,
            per_pixel,
            crate::map::paint::field::floor(by_population),
        )
    ...
    fn descended() -> App {
        ...
        app.init_resource::<crate::map::galaxy::spawn::StarProfile>();
