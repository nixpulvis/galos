...
//! going and the search's own layers ([`crate::map::route::frontier`]) are drawn
...
/// make [`mark`] or the resources it takes public. [`crate::map::galaxy::walk`]'s draw
...
///
/// What that buys is [`crate::map::galaxy::walk`]'s: choosing the systems a cell draws
...
/// [`crate::map::galaxy::walk::adopt`] drops what was worked out about the old — and the
...
///
/// Its own message rather than a [`crate::map::search::Search`]: asking for a
...
/// told apart.
///
/// Its own resource rather than [`crate::map::search::SearchNote`], which answers a
...
impl Standstill {
    ...
    /// rather than trusted, as [`crate::map::bodies::Clock`]'s anchor is:
...
impl Filters {
    ...
    ///   cells holding them are gathered ([`crate::map::galaxy::blobs::Named`]);
    ...
    /// [`crate::map::galaxy::populated::PopulatedOrder`].
    ...
    /// still running is the one question ([`crate::map::route::fetch::fetch_route`]
    ...
    /// taken off with it: [`crate::map::route::follow_filters`] draws a line only
    ...
    /// [`crate::map::galaxy::visibility`]; this only says it is there to see.
    ///
...
/// zero goes further and takes it off the map: [`crate::map::galaxy::spawn`] never loads
/// what no filter admits, and [`crate::map::galaxy::walk`]'s walk drops what already
...
impl DimTo {
    ...
    /// rings are painted flat in screen space with egui, taking their color
    ...
    /// ([`Filtering::excluded_are_drawn`], [`crate::map::galaxy::spawn`],
    /// [`crate::map::galaxy::walk`]) read a dropped system off either the position or
...
/// slots, [`crate::map::galaxy::spawn::spawn`] being at the limit.
#[derive(SystemParam)]
...
/// of those is a whole-table swap by [`crate::map::index::refresh`], and it reaches a
/// drawn system by way of [`crate::map::galaxy::walk::Republished`]: the swap comes with
