...
//! apart and the field blinks and swims; [`crate::map::paint::field`]'s module comment
//! argues that floor, and `crate::map::space` argues the placement that is not in
...
//!
//! - `crate::map::paint::field::build_field` — every system's mark, as one mesh
//! - [`draw_names`] — names, the grounds they are read against, the leaders
//! - [`crate::map::pointing::ring`] — the ring around what is pointed at
//! - [`crate::map::selection::ring`] — the ring around what is picked out
//! - [`crate::map::grid::draw_readouts`] — the ruled plane's three numbers, and
...
//! the plane moved, through the one-way channel `crate::map::ruled::read`
//! describes.
//!
...
//! drawn pixel radius is `crate::map::paint::field::drawn_radius` and nothing else; a
//! body's position is [`Places::seen`] and a system's is [`System::position`].
...
use crate::map::pointing::{INDICATOR, Indicator, PointedAt};
use crate::map::schedule::MapSet;
use crate::map::schedule::PaintSet;
use crate::map::screen::{
    annotations_layer, depth, depth_of, outline, screen_position,
    world_per_pixel,
};
use crate::map::selection::{SELECTION, Selected};
...
pub(crate) fn plugin(app: &mut App) {
    ...
    app.add_systems(
        ...
        (choose_names, respawn)
            ...
            .after(crate::map::galaxy::visibility)
            .after(crate::map::paint::sizing::size_by_distance)
            .after(crate::map::paint::sizing::size_photometrically)
            // A name stands off the mark drawn around what it names, so it
            // wants the mark settled this frame rather than last.
            .after(crate::map::pointing::size_indicators),
    );
    ...
    app.add_systems(EguiPrimaryContextPass, draw_names.in_set(PaintSet::Map));
}
...
pub(crate) const GAP: f32 = 0.75;

/// How far a label sits above its star, in text heights
pub(crate) const RISE: f32 = 1.0;
...
pub(crate) const GROUND: Srgba = Srgba::new(0.03, 0.03, 0.05, 1.);
...
pub(crate) const GROUND_PAD: f32 = 0.3;
...
/// [`crate::map::paint::sizing`]), and `ln(energy)` is, up to a constant, the margin by
...
/// can paint it and [`crate::map::pointing`] can size the area that catches the
...
pub(crate) fn choose_names(
    ...
    wanted.extend(systems.iter().filter_map(|candidate| {
        ...
        let ahead = orbit.radius
            - depth(orbit, position) / crate::map::space::LIGHT_YEAR as f32;
        if selected {
    ...
    for (entity, body, indicator) in &bodies {
        ...
        // Where the body's outline lands, which is what its mark goes around
        // and so what its name stands off from. See [`outline`].
        let Some(drawn) =
            outline(orbit, cot_half_fov, viewport, seen, body.radius)
        else {
            continue;
        };
        let spot = drawn.at;
        ...
        // aim at it.
        let depth = depth_of(orbit, crate::map::space::metres(seen)).max(1.);
        let score = name_score(
            LabelWeight::Body {
...
///
/// Its own size, rather than the mark [`crate::map::pointing`] draws to aim at it.
...
/// there *is* its brightness (see [`crate::map::paint::sizing::size_photometrically`]) — so
...
/// be read.
pub(crate) fn name_rect(at: Vec2, name: &str, clear: f32) -> Rect {
...
pub(crate) fn name_rect_of(at: Vec2, letters: usize, clear: f32) -> Rect {
...
/// set, which [`draw_names`] paints and [`crate::map::pointing`] catches the
...
pub(crate) fn respawn(
    mut commands: Commands,
    named: Query<
        (Entity, &System, Option<&Children>, Has<crate::map::route::Hop>),
        With<Named>,
    >,
...
fn nameplate(words: String) -> impl Bundle {
    (
        ...
        // screen by [`crate::map::pointing`], from the same rectangle
...
pub fn draw_names(
    ...
    >,
    bodies: Query<
        (&Body, &Indicator, Has<PointedAt>, Has<Selected>),
        Without<Label>,
    >,
    places: Places,
) -> Result {
    ...
    for (words, child_of) in &tokens {
        ...
        let (at, clear, tint, leader) =
            ...
            } else if let Ok((body, indicator, pointed, selected)) =
                bodies.get(thing)
            {
                let Some(seen) = places.seen(thing, orbit) else { continue };
                // Where its outline lands, which [`choose_names`] laid the
                // name out from. See [`outline`].
                let Some(drawn) =
                    outline(orbit, cot_half_fov, viewport, seen, body.radius)
                else {
                    continue;
...
/// Shared with [`crate::map::galaxy::blobs`], which writes what a merged mark stands
/// for and must not look like something else.
pub(crate) fn naming() -> egui::FontId {
    egui::FontId::new(NAME_HEIGHT, egui::FontFamily::Monospace)
}

/// The colour a name comes out in, given how it is marked out.
pub(crate) fn marked_tint(pointed_at: bool, selected: bool) -> Srgba {
    if selected {
        SELECTION
    } else if pointed_at {
        INDICATOR
...
mod tests {
    ...
    fn a_plate_is_measured_at_the_width_it_sets() {
        for name in ["sol", "SOL", "Shinrarta Dezhra", "LHS 3447", "straße"] {
            let system = crate::map::galaxy::tests::named(1, name);
    ...
    fn stop(name: &str, away: f64) -> System {
        let mut system = crate::map::galaxy::tests::named(1, name);
    ...
    fn winner(app: &mut App) -> Entity {
        app.world_mut()
            .spawn((crate::map::galaxy::tests::named(1, "Sol"), Named))
            .id()
    }
