...
pub fn plugin(app: &mut App) {
    ...
    // and after [`crate::map::pointing::ring`], which is where the order between
...
///
/// Answers [`crate::map::pointing::INDICATOR`], and has to be told apart from it
...
/// [`crate::map::bodies::spawn::Places::of`]: the ring round a body does,
/// and so does the camera keeping one under itself. See
/// [`crate::map::camera::Carried`].
#[derive(Clone)]
...
impl Selection {
    ...
    /// that moves -- see [`crate::map::camera::Carried`].
...
/// change in what is picked out, and [`crate::map::galaxy::fetch::fetch_selected`] asks the
...
/// [`crate::map::filter`]'s `mark` last cut, and a system would be let go
/// of
...
pub(crate) fn nothing_clicked(
    ...
    // [`crate::map::galaxy::blobs`]. Nothing is drawn for it that this query can see,
...
///
/// Drawn from the same target [`crate::map::pointing::ring`] measures, so the two
...
///
/// A stop a route reaches is left to [`crate::map::pointing::ring`] while the map
...
pub(crate) fn ring(
    ...
    for (entity, body, _) in &inside {
        let Some(mark) = crate::map::pointing::body_mark(
            orbit,
            cot_half_fov,
            viewport,
            &places,
    ...
    for (system, mark, indicator, visibility, filtered, hop) in &selected {
        // A stop a route reaches is ringed by [`crate::map::pointing::ring`], in
        ...
        // empty sky. Gated as the name is in [`crate::map::labels::choose_names`],
...
pub(crate) fn going(color: Srgba, standing: f32) -> Srgba {
...
mod tests {
    ...
    fn a_picked_body_is_told_where_it_stands() {
        ...
        assert_eq!(
            place,
            Some(crate::map::space::light_years(DVec3::new(1e11, 0., 0.))),
    ...
    /// stands for ([`crate::map::galaxy::blobs`]). Nothing on the map wears a
