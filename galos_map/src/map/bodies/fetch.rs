...
//!
//! Its own task rather than another arm of [`crate::map::galaxy::fetch`], which
...
/// anyway; read by [`crate::map::camera`]'s `zoom_floor`.
///
...
impl Approach {
    ...
    ///
    /// [`crate::map::camera`]'s `STOOD_IN`, which is the same distance for every
    ...
    pub fn stood_in(&self) -> bool {
        f64::from(self.away) * crate::map::space::LIGHT_YEAR
            <= f64::from(crate::map::camera::STOOD_IN)
    }
}
...
/// [`crate::map::bodies::spawn`]'s `WORTH_MARKING` is an angle, so a system reaching a
...
/// be an even one is for [`crate::map::grid`]'s `rule` to answer, not this.
