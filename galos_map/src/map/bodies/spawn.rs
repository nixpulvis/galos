...
pub fn plugin(app: &mut App) {
    ...
    app.add_systems(
        Update,
        draw.in_set(MapSet::Populate).after(crate::map::bodies::fetch::collect),
    );
    ...
    app.add_systems(
        ...
        relay
            ...
            .after(crate::map::camera::orbit_camera)
            .after(stand),
    );
    ...
    app.add_systems(
        Update,
        fade.in_set(MapSet::Camera).after(crate::map::camera::orbit_camera),
    );
}
...
/// in radians
///
/// The bottom of the ladder, and [`crate::map::paint::sizing`]'s `shell` is what
...
/// looking at. The rows arrive far earlier — that is [`crate::map::bodies::fetch`]'s
...
/// [`crate::map::paint::sizing::MARGIN`] is held under its inverse, so a mark is
...
///
/// Unit, so the scale [`crate::map::paint::sizing::size_marks`] writes is the arm
...
fn draw(
    ...
    // [`crate::map::grid`]), but holding the contents up until it finishes is a
    ...
    // [`crate::map::grid::rule`]'s to keep, and it keeps it in time rather than in
    // distance.
...
impl Places<'_, '_> {
    ...
    /// [`crate::map::camera::OrbitCamera`]). Asked by whatever has to tell two
...
fn relay(
    ...
    // not. See [`crate::map::camera::OrbitCamera`].
    ...
    // the clock steps rather than runs (see [`crate::map::bodies::WITHIN`]), so there is
...
fn seen_across(camera: &OrbitCamera, lens: Option<&Projection>) -> f64 {
    2. * crate::map::camera::framed(camera.radius, lens) as f64
        * space::LIGHT_YEAR
}
...
fn follow(mut clock: ResMut<Clock>) {
    crate::map::bodies::mark_if_moved(&mut clock, |clock| {
        clock.follows(chrono::Utc::now())
    });
}
...
mod tests {
    ...
    fn ringed(
        ...
        let back = crate::map::camera::stand_back(
            (radius as f64 / space::LIGHT_YEAR) as f32,
            None,
        );
        app.world_mut().spawn((
            crate::map::camera::OrbitCamera::inside(
                position,
        ...
        app.add_systems(
            Update,
            (crate::map::camera::orbit_camera, read_ring).chain(),
        );
    ...
    fn stands(away: f64, out: DVec3) -> DVec3 {
        ...
        let system = app
            .world_mut()
            .spawn((crate::map::galaxy::tests::at(1, away), grid))
            .id();
        app.world_mut().spawn((
            Body {
                address: 1,
    ...
    fn carried_on(out: f64, back: f64, through: f64) -> (f64, f64) {
        ...
        app.world_mut().spawn((camera, crate::map::galaxy::tests::seeing()));
    ...
    fn a_wider_system_goes_out_from_further_off() {
        let wide = crate::map::galaxy::tests::reaching(1, 0., 2.1e15);
        let ordinary = crate::map::galaxy::tests::reaching(2, 0., 1.5e12);
    ...
    fn panned_away(mark: f32) -> App {
        use crate::map::bodies::{Clock, Contents, FetchState};
        use crate::map::galaxy::tests::reaching;
        ...
        app.insert_resource(Contents {
            ...
            revision: 0,
            state: FetchState::Known(SystemBodies {
                bodies: vec![crate::map::bodies::tests::body(1e11)],
                ..default()
            }),
        });
        app.add_plugins(crate::map::paint::sphere::plugin);
        app.add_systems(Startup, init_materials);
        app.add_systems(Update, draw);

        let map = app.world_mut().spawn_empty().id();
        app.insert_resource(crate::map::space::Map(map));
        ...
        app.world_mut().spawn((
            reaching(2, 1., crate::map::bodies::STAND_IN),
            Shell,
            Transform::default(),
            Strength(1.),
        ));
    ...
    /// [`crate::map::grid::rule`]'s to keep, and it keeps it in time.
    #[test]
