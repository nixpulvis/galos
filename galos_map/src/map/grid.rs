...
//!
//! `crate::map::ruled` draws the plane itself: its lines, and the numbers
...
///
/// A touch under the names ([`crate::map::labels::NAME_HEIGHT`]) and the
...
/// leaders are drawn flat; see [`crate::map::labels::draw_names`].
///
...
pub(crate) fn draw_readouts(
    ...
    // fix. Their `Visibility` is the one [`crate::map::galaxy::visibility`] wrote
    // this frame, which is the component
    // [`crate::map::selection::ring`] asks as well: one question, one
    ...
    //
    // Asked nothing beyond existing, as [`crate::map::pointing::hits`]
...
fn placed<'a>(
    ...
    // See [`crate::map::camera::OrbitCamera`].
...
fn rule(
    ...
    // still being stepped: [`crate::map::bodies::spawn::fade`] leaves
    ...
    let (across, orbit) = match camera {
        Some((orbit, lens)) => {
            (crate::map::camera::framed(orbit.radius, lens) as f64, Some(orbit))
        }
        None => (0., None),
    };
    ...
    // [`crate::map::bodies::spawn::fade`] says the fade is for.
    //
    // Only the one system the map is holding ever has a mark that goes out
    // (see [`crate::map::bodies::spawn::fade`]), so a wide neighbour whose
    ...
    let wants = wanted
        ...
        .map(|(_, system, _, standing)| {
            ...
            let stood_in =
                away <= crate::map::paint::sizing::drawn_shell(system.reach());
...
mod tests {
    ...
    fn the_ruler_changes_hands_as_the_mark_goes_out() {
        ...
        for reach in [2.1e15_f32, STAND_IN] {
            let system = crate::map::galaxy::tests::reaching(1, 0., reach);
            ...
            let at = |seen: f32| {
                ...
                let stood_in =
                    away <= crate::map::paint::sizing::drawn_shell(reach);
    ...
    fn the_ruler_is_not_handed_over_past_what_the_galaxy_can_rule() {
        let grid = crate::map::space::galaxy_grid();
    ...
    fn the_ladder_stops_at_the_finest_cell() {
        let finest = finest(LIGHT_YEARS, &crate::map::space::galaxy_grid());
    ...
    fn the_window_reaches_past_the_ruling() {
        for shape in [0.25, 0.5, 1., 2.] {
            ...
            let framed = crate::map::camera::framed(1., Some(&lens)) as f64;
            for across in zooms() {
    ...
    fn a_system_rules_finer_than_the_galaxy() {
        let galaxy = finest(LIGHT_YEARS, &crate::map::space::galaxy_grid())
            * LIGHT_YEARS.metres;
        let system = finest(LIGHT_SECONDS, &crate::map::space::system_grid())
    ...
    fn a_ruling_inside_a_system_is_read_off_that_system() {
        ...
        let grid = crate::map::space::system_grid();
        let space =
