...
/// [`crate::map::route::follow_filters`] has nothing to say about it and the two lines
...
pub fn spawn_route(
    ...
    let (cell, translation) =
        grid.translation_to_grid(crate::map::space::metres(midpoint));
    ...
    let path = crate::map::route::Path::new(
        stops
            .iter()
            .map(|(address, at)| {
                (*address, crate::map::space::metres(*at - midpoint).as_vec3())
            })
            .collect(),
        flown.clone(),
    );

    // Whole to begin with, every stop taken as drawn. `crate::map::route::trim` cuts it
    ...
    commands.spawn((
        ...
        // wanted yet: `crate::map::route::trim` cuts it to the map and to the view
        // before the frame is presented.
        Mesh3d(meshes.add(crate::map::route::legs(&whole, &shown, &flown, 0.))),
        ...
        // [`crate::map::route::emphasise`] settles it from there.
        MeshMaterial3d(materials.add(StandardMaterial {
            base_color: line_color(crate::map::route::strength(true)),
