...
//! [`crate::map::grid::draw_readouts`]. Projected on the processor in `f64` from the
...
use crate::map::ruled::DistanceUnit;
use bevy::math::DVec3;
use bevy::prelude::*;
...
/// as the camera comes level with it. What [`crate::map::ruled::Plane::edge_on`] is set to
...
pub struct Reading {
    ...
    /// among the bodies. See [`crate::map::camera::OrbitCamera`].
    pub from: DVec3,
...
/// decide; put this on it and `crate::map::grid::draw_readouts` says where it is.
#[derive(Component)]
pub struct Located;
