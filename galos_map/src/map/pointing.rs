...
pub fn plugin(app: &mut App) {
    ...
    // way [`crate::map::labels::draw_names`] paints the names. A ring drawn as a
    ...
    // [`crate::map::selection::ring`], then the names. Before the names so both
...
fn triangle(hop: &crate::map::route::Hop, at: Vec2) -> Vec<[Vec2; 2]> {
    let way = match hop {
        crate::map::route::Hop::Next => 1.,
        crate::map::route::Hop::Last => -1.,
    };
    let wing = ICON * 0.5;
    let half = ICON * 0.35;
...
///
/// A hair bolder than the leader lines [`crate::map::labels::draw_names`] paints at
...
/// display density.
pub(crate) const DRAG_THRESHOLD: f32 = 5.;
...
#[derive(Component, Default)]
pub(crate) struct DragDistance(pub f32);
...
pub(crate) fn point_at(
    ...
    peopled: Query<(&System, Has<crate::map::route::Hop>)>,
    view: Res<View>,
    ...
    // what puts it into the ranking below; see [`crate::map::paint::sizing::by_population`].
    let by_population =
    ...
    // [`crate::map::paint::sizing::drawn_population`] and not the row's own count. Off,
...
/// come apart.
///
/// Taken from the pixels [`crate::map::paint::field`] paints the star at rather than
...
pub fn size_indicators(
    ...
    // around cannot come apart; see [`crate::map::paint::field::floor`].
    scale_population: Res<ScalePopulation>,
    mut systems: Query<
    ...
    for (system, shell, shown, mark, mut indicator) in &mut systems {
        ...
        let away = crate::map::space::metres(
            orbit.eye_from(DVec3::from(system.position)),
        )
        .length() as f32;
...
///
/// The pixels the field paints the star at ([`crate::map::paint::field::drawn_radius`]) and
...
fn system_mark(
    ...
    let star = crate::map::paint::field::drawn_radius(
        view,
        drawn,
        per_pixel,
        crate::map::paint::field::floor(scaling_by_population),
    )
    .unwrap_or(0.);
...
/// name stands off what it names, and `crate::map::labels::choose_names` packs the
...
fn hits(
    ...
    for (pointer, at) in &pointers {
        ...
        for (entity, system, indicator, drawn) in &systems {
            ...
            // same one [`crate::map::labels::choose_names`] laid out to keep names
        ...
        // [`crate::map::labels::draw_names`] and the two rings read it. Taken from
        ...
        for (entity, body, indicator) in &bodies {
            ...
            let hit =
                caught(eye, depth_of(orbit, crate::map::space::metres(seen)));
...
pub fn point_the_cursor(
    ...
    // holding tens of thousands of them as a list; see [`crate::map::galaxy::blobs`].
    blob: Res<crate::map::galaxy::blobs::PointedBlob>,
...
/// [`crate::map::labels::draw_names`] paints the names and the leaders that join a
...
/// [`crate::map::selection`]'s `ring` does and for the same reason.
...
pub fn ring(
    ...
    // which is what a stop is reached from and what [`crate::map::selection::ring`]
    // stands back for.
    if holding.of().is_some() {
        for (system, mark, indicator, hop, picked) in &hops {
            ...
            // there in another.
            let hue = if picked {
                crate::map::selection::SELECTION
            } else {
                crate::map::route::HOP
            };
            let color = crate::map::selection::going(hue, standing);
            ...
            let icon = match landed {
                ...
                Some(place) => {
                    ...
                    let left = indicator.0
                        + crate::map::labels::NAME_HEIGHT
                            * crate::map::labels::GAP;
    ...
    for (system, visibility, mark, indicator, filtered) in &pointed_at {
        // Gated as [`crate::map::selection::ring`] is, and for the reason argued
        ...
        painter.circle_stroke(
            egui::pos2(at.x, at.y),
            indicator.0,
            stroke(crate::map::selection::going(
...
mod tests {
    ...
    fn seen(radius: f32, depth: f32) -> Silhouette {
        let offset =
            DVec3::new(0., 0., -(depth as f64)) / crate::map::space::LIGHT_YEAR;
    ...
    fn sized() -> App {
        ...
        let mut standing = crate::map::galaxy::tests::system(1);
        standing.position = [0., 0., -5.];
        let wide = (0.034 * crate::map::space::LIGHT_YEAR) as f32;
        app.world_mut().spawn((
            standing,
            Shell,
            Indicator::default(),
