...
pub fn plugin(app: &mut App) {
    ...
    app.add_plugins(plan::plugin);
    app.add_plugins(walk::plugin);
    app.add_plugins(spawn::plugin);
    app.add_plugins(blobs::plugin);
    app.add_plugins(populated::plugin);
    ...
    app.add_systems(
        ...
        (zoom_with_spyglass, reach_with_camera)
            ...
            .after(crate::map::camera::move_camera)
            .before(crate::map::camera::orbit_camera),
    );
...
pub(crate) struct System {
    ...
    pub(crate) body_count: Option<i32>,
    pub(crate) non_body_count: Option<i32>,
...
impl System {
    ...
    /// the shell drawn at `crate::map::paint::sizing::MARGIN` of it always encloses what is
    /// inside, and the mark [`crate::map::paint::sizing::shell`] draws instead is what keeps a
    /// small system visible from a distance.
    ///
    /// [`crate::map::paint::sizing::shell`]: crate::map::paint::sizing
    pub(crate) fn reach(&self) -> f32 {
        self.reach.unwrap_or(crate::map::bodies::STAND_IN)
    }
...
impl Spyglass {
    ...
    pub(crate) const OPENING: f32 = 10.;
    ...
    pub(crate) const FLOOR: f32 = 1e-3;
    ...
    pub(crate) const CEILING: f32 = 1.1e5;
    ...
    pub(crate) const UNASKED: f32 = 200.;
}
...
impl Spyglass {
    ...
    /// first.
    pub(crate) fn reaches(&self, center: DVec3, position: DVec3) -> bool {
    ...
    /// at all.
    pub(crate) fn locks_camera(&self) -> bool {
...
/// ([`crate::map::paint::sizing::ScalePopulation`]): somebody has to live there. In that mode how
...
pub(crate) fn visibility(
    ...
    mut systems: Query<(
        &System,
        &mut Visibility,
        Has<crate::map::filter::Filtered>,
        Has<crate::map::route::Hop>,
        Has<big_space::prelude::Grid>,
    )>,
    spyglass: Res<Spyglass>,
    dim: Res<crate::map::filter::DimTo>,
...
/// child list each time (see `crate::despawn`), so dropping thousands would
...
pub(crate) fn drain_evictions(
    galaxy: Res<crate::map::space::Galaxy>,
...
pub(crate) fn zoom_with_spyglass(
    spyglass: Res<Spyglass>,
    ...
    if spyglass.locks_camera() {
        if let Ok(mut camera) = camera.single_mut() {
            camera.target_radius =
                crate::map::camera::opening_radius(spyglass.radius);
        }
    }
}
...
/// in [`crate::map::camera::FRAMING_MARGIN`] more than the route, which is a
...
pub(crate) mod tests {
    ...
    fn map(radius: f32, clear: bool, dim: f32) -> App {
        ...
        app.insert_resource(crate::map::filter::DimTo(dim));
        app.insert_resource(crate::map::paint::sizing::View::Map);
        app.insert_resource(crate::map::paint::sizing::ScalePopulation(false));
        app.init_resource::<InReach>();
    ...
    fn scaling_by_population_hides_an_empty_system() {
        let mut app = map(10., true, 0.15);
        app.insert_resource(crate::map::paint::sizing::ScalePopulation(true));
        let empty =
    ...
    fn the_realistic_sky_draws_an_empty_system() {
        let mut app = map(10., true, 0.15);
        app.insert_resource(crate::map::paint::sizing::ScalePopulation(true));
        app.insert_resource(crate::map::paint::sizing::View::Realistic);
        let empty =
            app.world_mut().spawn((peopled(1, 0), Visibility::default())).id();
    ...
    fn following(back: f32) -> f32 {
        crate::map::camera::framed(back, None) * (100 - FOLLOW_MARGIN) as f32
            / 100.
    }
    ...
    fn the_reach_stops_short_of_what_the_camera_sees() {
        ...
        let (reach, _) = linkage(&mut app);
        let seen = crate::map::camera::framed(100., None);
    ...
    fn a_framed_route_is_held_by_the_reach_taken_from_it() {
        for extent in [10., 50., 150.] {
            let back = crate::map::camera::stand_back(extent, None);
