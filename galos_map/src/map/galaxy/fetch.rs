...
pub fn plugin(app: &mut App) {
    ...
    // without it.
    app.init_resource::<crate::map::search::Plot>();
...
/// [`crate::map::bodies::fetch`] asks a system's interior again,
/// [`crate::map::filter`] re-cuts the time filter as its span slides, and
/// [`crate::map::index::refresh`] picks up an index that has been published again.
#[derive(Resource)]
...
impl Poll {
    ...
    /// it. [`crate::map::bodies::fetch`] asks about the inside of one
    /// system and [`crate::map::index::refresh`] asks whether what the map reads from has
...
/// do and [`crate::map::galaxy::spawn`] has one queue to drain.
///
...
pub(crate) mod tests {
    ...
    fn plotting() -> (App, crate::testing::Scratch) {
        ...
        // both. See [`crate::map::filter::Plotted`].
        app.init_resource::<crate::map::filter::Filters>();
        app.init_resource::<crate::map::route::SelectedFilter>();
    ...
    /// only ever asking now — stopping is [`crate::map::search::Search::Stop`]'s
    ...
    fn a_stop_takes_back_every_leg_of_a_trip() {
        ...
        app.world_mut().write_message(crate::map::search::Search::Stop);
        app.update();

        assert!(legs(&app).is_empty(), "a leg was left searching");
        assert_eq!(
            *app.world().resource::<crate::map::search::Plot>(),
            crate::map::search::Plot::Nothing,
    ...
    fn rows(app: &App) -> Vec<(String, Option<Plotted>)> {
        app.world()
            .resource::<crate::map::filter::Filters>()
            .iter()
    ...
    fn a_trip_stopped_part_way_keeps_a_row_for_every_leg() {
        ...
        assert_eq!(
            app.world().resource::<crate::map::filter::Filters>().routed(),
    ...
    fn row_of(app: &App, at: usize) -> crate::map::filter::Filter {
        app.world()
            .resource::<crate::map::filter::Filters>()
            .get(at)
            .expect("a row")
    ...
    fn again(app: &mut App, route: crate::map::filter::Filter) {
    ...
    fn a_faction_cannot_be_asked_again() {
        ...
        again(
            &mut app,
            crate::map::filter::Filter::Faction {
                id: 7,
    ...
    fn asking_for_a_route_takes_back_what_was_picked_out() {
        let (mut app, _dir) = plotting();
        app.insert_resource(crate::map::route::SelectedFilter(vec![
            crate::map::filter::Filter::Faction {
                id: 7,
                name: "Some Lot".to_owned(),
            },
        ...
        assert!(
            app.world()
                .resource::<crate::map::route::SelectedFilter>()
                .0
                .is_empty(),
        );
    }
