...
/// (see [`crate::map::paint::sizing::size_photometrically`]) both enlarges the stars already
...
/// per-vertex glint reads it (see [`crate::map::paint::field`]). The tint is the bucket's
...
/// [`crate::map::paint::sizing::CORE_WIDTHS`] out: at 256 across and 32 core widths of
...
pub(crate) fn star_psf(profile: ProfileKind) -> Image {
    ...
    // which is the reach `crate::map::paint::field` maps onto a star's quad and
    // `crate::map::paint::sizing::psf_draw` caps a disc at.
    let alpha = f64::from(n) / 2. / crate::map::paint::sizing::CORE_WIDTHS;
    let psf = crate::map::paint::sizing::instrument(profile, alpha as f32);
...
/// wear the same instrument. [`crate::map::paint::field`] samples it per mark in the
...
pub(crate) struct StarSprite {
    /// What [`star_psf`] cut, under the handle [`crate::map::paint::field`] cloned.
...
/// The map sizes a star by [`crate::map::paint::sizing`]'s own `psf_radius` law either way,
...
/// means. One color each, and nothing indexes them: [`crate::map::paint::field`] asks
/// `Hue::light` for the three channels it paints a mark with, and
/// [`crate::map::paint::glow`] for the three it weights a cell's political histogram by.
...
impl Hue {
    ...
    /// hue comes out is the gains' to say — [`crate::map::paint::glow::Gains::mark`] for
    /// a mark, and [`crate::map::paint::glow::Gains::faint`] and
    /// [`crate::map::paint::glow::Gains::unaligned`] for how much of one an uninhabited
    ...
    /// the ungoverned galaxy was invisible. [`crate::map::paint::glow`]'s backdrop
...
/// map view's [`crate::map::labels::NameRadius`] or the realistic view's
/// [`crate::map::labels::NameLimit`], and by the room each is given, rather
...
/// painted flat in screen space by [`crate::map::paint::field`], off this entity's
/// position and the size [`crate::map::paint::sizing`] writes onto it. That size is an
...
#[derive(Component)]
#[require(crate::map::paint::sizing::Drawn)]
pub struct Shell;
...
fn select_on_click(
    ...
    *answered = Some(frame.0);
    ...
    // [`crate::map::selection::clear_when_nothing_is_clicked`] is what answers it.
...
/// thread (see [`crate::map::galaxy::fetch`]), so nothing here joins a table or clones a
...
/// flight ([`crate::map::galaxy::flight`]): a queue pinned at 63,488 entries and
...
/// [`crate::map::labels::draw_names`]), so nothing has to undo the exaggerated
/// size [`crate::map::paint::sizing`] writes here.
///
/// A row already on the map has its [`System`] replaced rather than being
/// respawned, which [`update`] then acts on.
///
/// The filters are asked here rather than left to [`crate::map::filter`]'s `mark`, so that a
...
pub fn spawn_systems(
    ...
    for system in new_systems {
        ...
        // not paid and then hidden. Left to [`crate::map::galaxy::walk`]'s walk to
...
fn star(system: System, grid: &Grid, galaxy: Entity) -> Star {
    (
        ...
        // `crate::map::paint::sizing` writes onto it, so there is no mesh, material or
...
fn placement(system: &System, grid: &Grid) -> (CellCoord, Transform) {
    let (cell, translation) = grid.translation_to_grid(
        crate::map::space::metres(DVec3::from(system.position)),
    );
...
///
/// One image for the whole sky, put up before [`crate::map::paint::field`]'s own startup
