...
pub fn plugin(app: &mut App) {
    ...
    // The screenshot path, if one was asked for. Not gated on the panel: what
    // it is for is verifying a render change from outside the window.
    app.add_systems(
        Update,
        shoot.run_if(
            resource_exists::<Shot>
                .and_then(in_state(crate::loading::Opening::Drawn)),
        ),
    );
}

/// Write a PNG of the map and exit
///
/// There is no way to read what the map draws from outside the process: a
/// render change is judged by looking at it, and the window is the only place
/// it exists. So the map can be asked to draw one frame and hand it over,
/// which is what makes a field, a mark size or a blend something a change can
/// be checked against rather than argued about.
///
/// `radius` stands the camera off Sol before the shot, since what a far field
/// looks like and what a near one does are different pictures. Frames are
/// waited out rather than trusted: the camera eases toward its target
/// ([`OrbitCamera::zoom_smoothness`]) and the walk, the fetch and the spawn
/// each act on the frame before, so a capture on the first drawn frame catches
/// a map that has not finished arriving.
#[derive(Resource)]
pub struct Shot {
    /// Where the PNG is written.
    pub path: String,
    /// How far to stand the camera off before the shot, in light years.
    pub radius: Option<f32>,
    /// Frames to let the map settle first.
    pub after: u32,
}

/// Take the shot once the map has settled, then leave
fn shoot(
    ...
    *waited += 1;
    // The stand-off is asked for once, on the first drawn frame, and then the
    // frames below are what lets the ease actually arrive at it.
    //
    // The panel comes up with it, so the numbers are in the picture: what a
    // field looks like and what it laid down are the two halves of reading a
    // render change, and a shot with only the first is a shot that has to be
    // guessed at.
    if *waited == 1 {
        show.0 = true;
        if let (Some(radius), Ok(mut camera)) =
            (shot.radius, camera.single_mut())
        {
            camera.radius = radius;
            camera.target_radius = radius;
        }
    }
    if *waited == shot.after {
        commands
            .spawn(bevy::render::view::screenshot::Screenshot::primary_window())
            .observe(bevy::render::view::screenshot::save_to_disk(
                shot.path.clone(),
            ));
        info!("wrote {} with the field at {glow:?}", shot.path);
    }
    // The capture is handed to the render world and written from an observer,
    // so the frames after it are what the file needs to exist.
    if *waited >= shot.after + 30 {
