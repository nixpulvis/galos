...
pub(crate) fn outline(
    ...
    let away = crate::space::metres(offset);
...
pub(crate) fn choose_names(
    ...
    for (entity, body, indicator) in &bodies {
        ...
        // aim at it.
        let depth = depth_of(orbit, crate::space::metres(seen)).max(1.);
        let score = name_score(
            LabelWeight::Body {
...
mod tests {
    ...
    fn the_flat_projection_is_the_one_the_scene_is_drawn_with() {
        ...
        for offset in [
            ...
            let painted = screen_offset(
                ...
                cot_half_fov,
                viewport,
                (rotation * offset).as_dvec3() / crate::space::LIGHT_YEAR,
            )
    ...
    fn a_ball_aside_draws_its_outline_past_its_middle() {
        ...
        let offset =
            DVec3::new(depth * 0.29, 0., -depth) / crate::space::LIGHT_YEAR;
