...
pub fn plugin(app: &mut App) {
    ...
    app.add_systems(Update, capture.after(crate::systems::field::build_field));
}
...
fn capture(
    ...
    systems: Query<(
        &System,
        &Visibility,
        &crate::systems::scale::Drawn,
        &crate::systems::bodies::spawn::Strength,
    )>,
    mut view: ResMut<crate::systems::scale::View>,
    mut exposure: ResMut<crate::systems::spawn::StarExposure>,
    mut spyglass: ResMut<crate::systems::Spyglass>,
    ...
    {
        let floor = crate::systems::field::floor(false);
        ...
        for (system, visibility, drawn, strength) in systems.iter() {
            ...
            let away =
                crate::space::metres(camera.eye_from(at)).length() as f32;
            let per_pixel = crate::systems::labels::world_per_pixel(
                cot,
                height,
                away.max(1.),
            );
            if crate::systems::field::drawn_radius(
