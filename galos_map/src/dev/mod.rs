...
use crate::map::galaxy::fetch::FetchTasks;
use crate::map::galaxy::spawn::PendingSpawns;
use crate::map::galaxy::{
    Evictions, InReach, PendingEvictions, Spyglass, System,
};
use crate::map::index::{Factions, IndexDir, Names, Populated, ResidentIndex};
...
#[derive(SystemParam)]
struct Descent<'w, 's> {
    contents: Res<'w, crate::map::bodies::Contents>,
    handover: Res<'w, crate::map::grid::Handover>,
    systems: Query<'w, 's, (&'static System, Has<big_space::prelude::Grid>)>,
    bodies: Query<'w, 's, (), With<crate::map::bodies::spawn::Inside>>,
    ...
    planes: Query<
        'w,
        's,
        (&'static crate::map::grid::Ruler, &'static crate::map::ruled::Reading),
    >,
}
...
pub fn plugin(app: &mut App) {
    ...
    // read, which until then is nothing. See [`crate::map::index::load`].
    app.add_systems(
...
struct Drawing<'w> {
    glow: Res<'w, crate::map::paint::glow::Laid>,
    sampled: Res<'w, crate::map::galaxy::walk::Sampled>,
}
...
/// [`crate::map::galaxy::visibility`] already settled), and the eviction and fetch
                    ...
                    pair(
                        ...
                        &nearest.map_or(
                            ...
                            |(system, _, orbit)| {
                                ...
                                .length()
                                    as f32;
                                let shell = crate::map::paint::sizing::drawn_shell(
                                    system.reach(),
                                );
                    ...
                    pair(
                        ...
                        &nearest.map_or(
                            ...
                            |(system, _, orbit)| {
                                format!(
                                    "{:.2} left",
                                    crate::map::bodies::spawn::standing_for(
                                        system,
                                        orbit.eye(),
                                    )
                                )
...
/// ([`crate::map::galaxy::reach_with_camera`]), so inside a system it is
...
fn reach(light_years: f32) -> String {
    if light_years >= 1. {
        ...
        let seconds = f64::from(light_years) * crate::map::space::LIGHT_YEAR
            / 299_792_458.;
        format!("{seconds:.0} Ls")
    }
}
