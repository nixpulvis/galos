...
//! `galos-sync --db --index DIR` from what EDDN carries. That is everyone
//! else's game, and it is missing exactly one commander's: the systems they
//! have been to that nobody has reported, the bodies only they have scanned,
//! and the jump they made thirty seconds ago. The game already writes all of
//! it, to a directory of `.log` files, on the same machine the map runs on.
//!
...
//! directory — [`JournalSource::publish`], which is what `galos-sync --from
//! journal=JOURNAL --index DIR` does — for looking at one commander's own
//! sky on its own.
//!
