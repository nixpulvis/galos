...
//!
//! A directory is produced two ways. [`crate::cold`] raises one from
//! records, which `galos_db::index` reads out of Postgres over every system
//! anybody has ever reported; [`crate::galaxy`] accumulates one from events,
...
//! client, and a running `galos-sync` maintains one directory from both at
//! once. Where they read the same fact they must read it the same way: a
...
/// payload carries.
///
/// One reading of `at`, so the two cannot disagree about a system. The
/// buckets are days wide, where the map's Recency control goes down to a
...
///
/// `stars` is the `(visual absolute magnitude, temperature)` of every
/// scanned star — visual, the caller having already carried Elite's
/// bolometric figure through [`Magnitude::visual`], which is where a white
/// dwarf keeps its faint brightness and a black hole falls to nothing. Their
/// light adds, so the magnitudes combine to the one figure the sky is
/// ordered by, and the tint is the brightest star's, that being the one that
/// dominates what the pair looks like. With no stars `fallback_class` stands
/// in, and with no class [`ClassLight::of`] hands back the M dwarf the
/// galaxy is mostly made of.
///
/// Folded in one pass, no vector in between: the flux sums as it goes and
/// the brightest star is carried along. A tie keeps the first star.
pub fn lit(
...
/// one distance from arrival to the resolution the journal prints.
///
/// [`None`] for a system with no star on record, which leaves the caller to
/// its own fallback — the database side coalesces to the
/// `primary_star_class` column and the journal side to the class a plotted
/// route named. The same rule stands in SQL as the `DISTINCT ON
...
mod tests {
    ...
    ///
    /// The unscanned case is nearly every system there is.
    #[test]
    ...
    ///
    /// A magnitude is inverted and logarithmic: the pair must come out
    /// brighter than its brightest member, and wear that member's
    /// temperature.
    #[test]
