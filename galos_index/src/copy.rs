...
//! [`crate::source`] tomorrow is backed up without this module being
...
//! (see [`crate::checkpoint`]) — so a copy that took the directory alone
...
//!   [`sweep_payloads`](crate::sweep_payloads) makes for running after the
...
//! - **Not the scratch.** [`names::scratch_dir`](crate::names::scratch_dir)
//!   — `names/.building/` — is a fold in progress, and
//!   [`cold::spill_dir`](crate::cold::spill_dir) —
...
//! construction — [`crate::pack`] punches the dead runs out of
...
use crate::cold::mark_path;
use crate::names::scratch_dir;
...
///
/// The base is [`checkpoint::beside`], which is the workspace's one
...
fn siblings(dir: &Path) -> [PathBuf; 3] {
    let base = checkpoint::beside(dir);
    let log = Pending::path(&base);
...
mod tests {
    use super::*;
    use crate::Lock;
    use crate::checkpoint::{By, Checkpoint};
    use crate::store::PAYLOAD_DIR;
    ...
    fn the_lock_is_not_copied() {
        ...
        assert!(Lock::path(&from).exists(), "the writer still holds it");
        assert!(!Lock::path(&to).exists(), "the copy took the claim with it");
    ...
    fn occupied_sees_a_sibling_with_no_directory() {
        ...
        fs::write(checkpoint::beside(&dir), b"a base, no tree").unwrap();
        ...
        removed(&checkpoint::beside(&dir)).unwrap();
    ...
    fn discard_takes_the_siblings_and_leaves_the_lock() {
        ...
        assert!(!occupied(&dir));
        assert!(Lock::path(&dir).exists(), "the caller's claim was dropped");
        drop(held);
