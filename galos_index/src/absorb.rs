...
//! [`SystemReport::over`](crate::report::SystemReport::over) for a system's
//! own columns and [`crate::merge`] for the things inside it; both use `>=`
...
//! itself. A [`NameEntry`](crate::meta::NameEntry) carries no stamp at all,
...
//! out again over the merged contents by the calls [`crate::galaxy`] makes.
...
//! entirely ([`crate::checkpoint`], `src/sink/index.rs`) — so a merge that
...
use crate::bodies::{Bodies, Published};
...
use crate::merge;
use crate::meta::{
    Barycenter, Body, Boost, Faction, PopulatedSystem, Star, StarKind, Surface,
    SystemBodies, SystemBoost, SystemReach,
};
use crate::names::Names;
use crate::pack;
use crate::sidecars::{Moved, Sidecars};
use crate::source;
use crate::store;
...
pub fn absorb(
    ...
    for dir in [into, from] {
        if let Some(version) = store::stale(dir) {
...
///
/// [`crate::cold`]'s own `step`, in this module's vocabulary: a merge over a
...
fn table<T: DeserializeOwned>(path: &Path) -> Result<Vec<T>, Refused> {
    match source::read_meta::<Vec<T>>(path) {
        Ok(rows) => Ok(rows),
...
fn factions(dir: &Path) -> Result<Vec<Faction>, Refused> {
    table(&source::factions_path(dir))
}
...
/// first written (`crate::galaxy`, and [`Sidecars::add_factions`], which
...
/// calls [`crate::galaxy`] makes and not by arithmetic of this module's own
...
fn unite(
    ...
    if let Some(base) = out.take() {
        base.finish(union.cursor, held.by)
            .map_err(failed("the merged resume point"))?;
        match std::fs::remove_file(into.join(store::INDEX_FILE)) {
...

    {
        let mut fold = |stood: &System| -> Result<(), Refused> {
            ...
            if arrived && !relit.contains_key(&address) {
                let inside = source::read_bodies(into, address)
...
///
/// A [`NameEntry`](crate::meta::NameEntry) carries no stamp, so it cannot
...
fn carry_sidecars(
    ...
    let populated: Vec<PopulatedSystem> = table(&source::populated_path(from))?;
    let reaches: Vec<SystemReach> = table(&source::reaches_path(from))?;
    let boosts: Vec<SystemBoost> = table(&source::boosts_path(from))?;
...
/// ([`pack::each_address`]), one shard index at a time. A directory still
...
/// through [`source::read_bodies`], which answers out of whichever of the
    ...
    let finished = {
        let mut each = |address: i64| {
            ...
            let read = || -> io::Result<SystemBodies> {
                let said = source::read_bodies(from, address)?;
                let stood = source::read_bodies(into, address)?;
        ...
        pack::each_address(from, stop, &mut each)
...
///
/// [`crate::merge`] is the scan-over-stored twin of this and states the rule
...
mod tests {
    use super::*;
    use crate::cold::Taking;
    use crate::meta::NameEntry;
    use crate::sidecars::{write_boosts, write_reaches};
    ...
    /// [`LEAF_CAP`](crate::tree::LEAF_CAP) systems, so the tree the
    ...
    fn what_is_a_function_of_the_contents_is_worked_out_again() {
        ...
        pack::write_each(&into.0, [(1i64, &stood)]).expect("what stood");
        pack::write_each(&from.0, [(1i64, &arriving)]).expect("what arrived");
        ...
        let merged = source::read_bodies(&into.0, 1).expect("the bodies");
        ...
        let boosts: Vec<SystemBoost> =
            source::read_meta(&source::boosts_path(&into.0))
        ...
        let reaches: Vec<SystemReach> =
            source::read_meta(&source::reaches_path(&into.0))
    ...
    fn a_system_scanned_on_both_sides_keeps_both_scans() {
        ...
        let merged =
            source::read_bodies(&into.0, 1).expect("the merged record");
    ...
    fn a_faction_numbered_twice_is_refused() {
        ...
        source::write_meta(&source::factions_path(&into.0), &ours)
            .expect("the standing factions");
        source::write_meta(&source::factions_path(&from.0), &theirs)
