...
//! written to Postgres and baked into a directory by `galos-sync --db
//! --index DIR`. A commander's own journal is the other half of the same sky
//! and never reaches that directory — the systems nobody has reported, the
//! bodies only this commander has scanned, the arrival that happened thirty
//! seconds ago. `galos_journal` turns that directory of `.log` files into the
//! same vocabulary, and this is where the two meet.
//!
...
//! `galos-sync --db --index DIR` owns and rewrites: the next full build would
//! drop them, a `--only` pass would drop half of them, and there would be no
//! way left to ask what the galaxy looks like without them. So the two
//! directories stay separate, whole, and independently rebuildable, and the
//! join is made in the reader, per call, over whatever each side says at that
//! moment. Which is also what makes the toggle possible at all: a merge is
//! undone by answering the next call differently, where a bake would have to
//! be rebuilt.
//!
//! ## How the two compose
//!
