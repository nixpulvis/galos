...
//! budget; [`crate::cold`] counts it in its report rather than hiding it.
//!
...
use crate::region::Cut;
use crate::spill::{Spill, Spilled, as_bytes};
...
/// Where a cell's systems are spilled.
///
...
pub fn path_of(dir: &Path, cell: CellId) -> PathBuf {
...
/// out in [`crate::ColdReport::over_budget`].
fn divide(
    dir: &Path,
    region: CellId,
    path: PathBuf,
...
fn split(
    ...
    for &system in systems {
        ...
        match pieces.entry(into) {
            ...
            Entry::Vacant(empty) => {
                empty.insert(Spill::create(&path_of(dir, into))?)
            }
        }
        .push(system)?;
    }
...
mod tests {
    use super::*;
    use crate::region::{Crown, Offer, joined};
