...
impl Galaxy {
    ...
    /// copy. See [`crate::accumulate::bodies`].
    ...
    /// answer the `--db` sink's write path gets, which is what stops the two
    ...
    ///
    /// The rule is [`crate::accumulate::merge`]'s, which is the write path's:
...
impl Rows {
    ...
    pub fn system(&mut self, galaxy: &Galaxy, address: i64) -> io::Result<()> {
