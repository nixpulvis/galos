...
//!
//! Not run at open, unlike [`crate::source::migrate`]'s resharding. That is
...
//! [`crate::serialization::Point`]'s width changed, and `Cell::LEN` did
...
use crate::serialization::{
    Decode as _, INDEX_VERSION, index_version, legacy_payload_points,
    payload_bytes, payload_head,
};
use crate::store::payload_path;
...
impl Kinds {
    ...
    fn swept(
        ...
        crate::pack::each_arrival_class(dir, stop, &mut |address, class| {
...
pub fn rewrite(
    ...
    for cell in index.cells() {
        ...
        crate::store::write_payload_bytes(
            dir,
            cell.id,
    ...
    // to finish.
    let placed = crate::source::place_boosts(dir)?;
...
fn read_any_version(dir: &Path) -> io::Result<Index> {
    let path = dir.join(crate::store::INDEX_FILE);
    ...
    for _ in 0..count {
        let cell = crate::aggregate::Cell::decode(&mut cur)
...
mod tests {
    use super::*;
    use crate::meta::{Star, SystemBodies};
    use crate::serialization::{Encode as _, payload_points};
    use crate::tree::{BuildParams, System, Tree};
    ...
    /// does *nothing* — [`crate::source::migrate`] asks
    /// [`crate::store::stale`] first and returns having named this command
    ...
    fn a_rewrite_places_the_supercharge_table() {
        ...
        let mut names = crate::names::Names::open(&dir).expect("a table");
        names.name(crate::meta::NameEntry {
            address: 7,
            name: crate::name::SystemName::new("SOL"),
            position: [at[0] as f32, at[1] as f32, at[2] as f32],
        });
        names.publish(&dir).expect("a published name");

        crate::source::write_meta(
            &crate::source::boosts_path(&dir),
            &vec![Unplaced { address: 7, boost: crate::meta::Boost::Neutron }],
        )
        ...
        // names table gave it.
        let table: Vec<crate::meta::SystemBoost> =
            crate::source::read_meta(&crate::source::boosts_path(&dir))
